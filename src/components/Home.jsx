...
const wellnessData = [
	{
		...
		category: 'home',
		image:
			'https://images.unsplash.com/photo-1645652367526-a0ecb717650a?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHx5b2dhJTIwbWVkaXRhdGlvbiUyMHdlbGxuZXNzfGVufDF8fHx8MTc2MTIyODQxOHww&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral',
		description:
			'Start your day with this energizing smoothie bowl topped with fresh fruits, granola, and a drizzle of honey. Perfect for a quick and healthy breakfast!',
		likes: 234,
		tags: [
			'banana',
			'spinach',
		...
		comments: [
			{
				id: 1,
				user: 'Sarah',
				text: 'This looks amazing! Definitely trying this tomorrow morning.',
				time: '2 hours ago',
			},
			{
				id: 2,
				user: 'Mike',
				text: 'Made this today and it was delicious! Added some chia seeds too.',
				time: '5 hours ago',
			},
		],
	},
	{
		...
		category: 'recipes',
		image:
			'https://images.unsplash.com/photo-1670164747721-d3500ef757a6?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxoZWFsdGh5JTIwZm9vZCUyMG51dHJpdGlvbnxlbnwxfHx8fDE3NjEyNTE2MDN8MA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral',
		description:
			'A colorful and nutritious bowl packed with quinoa, roasted vegetables, chickpeas, and tahini dressing. Ready in just 30 minutes!',
		likes: 567,
		tags: [
			'quinoa',
			'chickpeas',
		...
		ingredients: [
			...
			{
				item: 'Chickpeas',
				amount: '1 can (drained and rinsed)',
				isLeftover: true,
			},
	...
	},
	{
		...
		category: 'post',
		image:
			'https://images.unsplash.com/photo-1634788699201-77bbb9428ab6?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxmaXRuZXNzJTIwZXhlcmNpc2UlMjB3b3Jrb3V0fGVufDF8fHx8MTc2MTIzMDE2N3ww&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral',
		description:
			'High-protein pasta dish with grilled chicken, spinach, and a light garlic sauce. Perfect post-workout meal that tastes amazing!',
		likes: 423,
	...
	},
	{
		...
		category: 'home',
		image:
			'https://images.unsplash.com/photo-1716893917077-5b320c1ecfec?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxzcGElMjByZWxheGF0aW9ufGVufDF8fHx8MTc2MTI0NjU5NXww&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral',
		description:
			'Elevated avocado toast with poached eggs, cherry tomatoes, and microgreens. A brunch favorite that\'s ready in 10 minutes!',
		likes: 389,
	...
	},
	{
		...
		category: 'home',
		image:
			'https://images.unsplash.com/photo-1606579820984-1df8e02419a7?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxuYXR1cmUlMjBwZWFjZWZ1bCUyMHNjZW5lcnl8ZW58MXx8fHwxNzYxMjUxNDcxfDA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral',
		description:
			'Nourishing green soup with kale, broccoli, and herbs. Light yet filling, perfect for a healthy lunch or dinner.',
		likes: 612,
		tags: [
			'kale',
			'broccoli',
	...
	},
	{
		...
		category: 'like',
		image:
			'https://images.unsplash.com/photo-1646208160263-b0feaf01a4cd?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxtaW5kZnVsbmVzcyUyMHBlYWNlfGVufDF8fHx8MTc2MTI4NDc0OXww&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral',
		description:
			'No-bake chocolate energy bites made with dates, nuts, and cacao. Perfect healthy snack that satisfies your sweet cravings!',
		likes: 445,
...
export default function Home({ category, userRecipes, likedItems, onToggleLike, currentUser }) {
	const [selectedItem, setSelectedItem] = useState(null);
	const [localLikedItems, setLocalLikedItems] = useState(new Set());
	const [selectedTagFilters, setSelectedTagFilters] = useState([]);
	const [customTagInput, setCustomTagInput] = useState('');

	// Use likedItems instead of externalLikedItems
	console.log(likedItems);

	// Use passed-in likedItems if available, otherwise local state
	const effectiveLikedItems = likedItems ?? localLikedItems;

	// Use passed-in toggle handler if available, otherwise local handler
	const toggleLike =
		onToggleLike ??
		((id) => {
	...
	const allIngredientTags = [
		...new Set(allAvailableData.flatMap((recipe) => recipe.tags || [])),
	].sort();
		...
		selectedTagFilters.length > 0
			? filteredByCategory.filter((item) => {
					...
					return selectedTagFilters.every((filterTag) =>
						item.tags.some((recipeTag) =>
							recipeTag.toLowerCase().includes(filterTag.toLowerCase())
						)
					);
			  })
			: filteredByCategory;
					...
					<div className="space-y-4">
						<div>
							...
							<p className="text-sm text-gray-500 mt-1">
								Select ingredients to find recipes. Multiple tags = recipes with ALL
								ingredients.
							</p>
						</div>
							...
							<div className="flex flex-wrap gap-2">
								{commonIngredientTags.map((tag) => (
									<Badge
										key={tag}
										variant={
											selectedTagFilters.includes(tag) ? 'default' : 'outline'
										}
							...
							<div className="space-y-2">
								<div className="flex items-center justify-between">
									<p className="text-sm text-gray-600">
										Searching for: ({selectedTagFilters.length} ingredient
										{selectedTagFilters.length > 1 ? 's' : ''})
									</p>
									<Button
										variant="ghost"
										size="sm"
						...
						{selectedTagFilters.length > 0 && (
							<p className="text-sm text-gray-600">
								Found {filteredData.length} recipe
								{filteredData.length !== 1 ? 's' : ''} with{' '}
								{selectedTagFilters.length > 1 ? 'all' : 'this'} ingredient
								{selectedTagFilters.length > 1 ? 's' : ''}
							</p>
						)}
					</div>
			...
			{filteredData.length === 0 && category === 'like' && selectedTagFilters.length === 0 ? (
				<Card className="p-16 text-center">
					...
					<p className="text-gray-500 leading-relaxed">
						Start exploring recipes on the Home tab and like your favorites to see
						them here!
					</p>
				</Card>
							...
							<div className="relative aspect-[4/3] overflow-hidden">
								...
								{currentUser && userRecipeIds.has(item.id) && (
									<div className="absolute top-3 left-3">
										<Badge className="bg-emerald-600">Your Recipe</Badge>
									</div>
								)}
								{/* Show leftover count badge */}
								{item.leftoverIngredients &&
									item.leftoverIngredients.length > 0 && (
										<div
										...
										>
											<Badge className="bg-orange-500">
												🥕 {item.leftoverIngredients.length} leftover
												{item.leftoverIngredients.length > 1 ? 's' : ''}
											</Badge>
										</div>
									)}
							</div>
								...
								<div className="flex items-center gap-6 text-gray-500">
									...
									>
										...
										<span className="text-sm">
											{effectiveLikedItems.has(item.id) ? item.likes + 1 : item.likes}
										</span>
									</button>
